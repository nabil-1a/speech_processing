...
def main(config, epoch):
    ...
    for i, (mixture, _, _, names) in enumerate(dataloader):
        ...
        print("\tSTFT...")
        #mixture_mag, mixture_phase, mixture_real, mixture_imag = stft.transform(mixture)
        mixture_D = stft.transform(mixture)
        #print("mix_D _ ", mixture_D.shape)
        mixture_real = mixture_D[:, :, :, 0]
        #print("real _ ", mixture_real.shape)
        mixture_imag = mixture_D[:, :, :, 1]
        #print("imag _ ", mixture_imag.shape)
        mixture_mag = torch.sqrt(mixture_real ** 2 + mixture_imag ** 2)  # [1, T, F]
        #print("mix_mag _ ", mixture_mag.shape)

        print("\tEnhancement...")
        enhanced_mag = model(mixture_mag).detach().cpu().unsqueeze(0)  # [1, T, F]
        #print("enhance_mag _ ", enhanced_mag.shape)
        ...
        enhanced_D = torch.stack([enhanced_real, enhanced_imag], 4)
        enhanced_D = enhanced_D.squeeze(0)
        
        #print("enhance_real _ ", enhanced_real.shape)
        #print("enhance_imag _ ", enhanced_imag.shape)
        #print("enhance_D _ ", enhanced_D.shape)
        #enhanced_magnitude = torch.sqrt(enhanced_real ** 2 + enhanced_imag ** 2)
        #enhanced_phase = torch.atan2(enhanced_imag.data, enhanced_real.data)
        #enhanced = stft.inverse(enhanced_mag, enhanced_phase)
